...
from convertdf import readindf
from getboostedwords import getdist
        ...
        for j in range(i - HALFLEN ,i + HALFLEN + 1):
            ...
            if j >= 0 and j < len(probs):
                if bestsstructures[s][1][relativepos] == 'A':
                    ...
                    probs[j][1] += .33 * f(j - i) * logisticvalue
                    probs[j][2] += .34 * f(j - i) * logisticvalue
def splitunder(word,ind):
    return word.split('_')[ind]

def get_probs_ident(infilename,outfilename,dirnum,ka):
    ...
    fnum = infilename[37:-4]
    print('fnum: {}'.format(fnum))
    infile2 = open('predictions/236short/predictknn{}.txt'.format(fnum),'r').readlines()
    infile = open(infilename,'r').readlines()
    numn = determinek(infile)
    numn2 = determinek(infile2)
    k = ka
    k2 = 20
    ...
    for it in range(0,len(infile),numn):
        ...
        inds = [1,3,5]
        inds = [1,4,7]
        for num in range(it,it+k2):
            arr = infile[num].split()
            if arr[0].split('_')[0] == arr[inds[0]].split('_')[0] or arr[0].split('_')[0] == arr[inds[1]].split('_')[0] or arr[0].split('_')[0] == arr[inds[2]].split('_')[0]:
        ...
        inf2split = infile2[it // numn * numn2].split()
        otherwords = [splitunder(inf2split[a],0) for a in [1,3,5]]
        mindist = min(min(alphaknns),min(betaknns),min(coilknns))
        aamindist = min([float(inf2split[a]) for a in [2,4,6]])
        if mindist - aamindist > .05:
            alphaknns = [float(inf2split[2])]
            alphasstructures = [splitunder(inf2split[1],1)]
            alphaaa = [splitunder(inf2split[1],0)]
            betaknns = [float(inf2split[4])]
            betasstructures = [splitunder(inf2split[3],1)]
            betaaa = [splitunder(inf2split[3],0)]
            coilknns = [float(inf2split[6])]
            coilsstructures = [splitunder(inf2split[5],1)]
            coilaa = [splitunder(inf2split[5],0)]
        ...
        if ident == False:
            ...
            dis = neighbors[0][0] 
        for p in range(max(0,(it//numn)-11),min(it//numn + 11,len(infile)//numn)):
...
def get_probs_targets(infilename,outfilename,dirnum,ka):
    #this function needs to get all the pieces together from the infile and give them to add_probs
    print(infilename)
    print('before reading file',str(datetime.now().time()))
    infile = open(infilename,'r').readlines()
    ...
    for it in range(0,len(infile),numn):
        ...
        #sstructures = alphasstructures + betasstructures + coilsstructures
        if len(alphaknns) > 0:
            neighbors = sorted([(alphaknns[i],alphasstructures[i],alphaaa[i]) for i in range(len(alphaknns))] )
            knns = [neighbors[i][0] for i in range(k)]
            sstructures = [neighbors[i][1] for i in range(k)]
            aas = [neighbors[i][2] for i in range(k)]
            if ident == False:
...
def get_probs(infilename,outfilename,dirnum,ka):
    df = readindf(open('dfs/alphadis{}.h'.format(23),'r').readlines())
    #this function needs to get all the pieces together from the infile and give them to add_probs
    print(infilename)
    print('before reading file',str(datetime.now().time()))
    infile = open(infilename,'r').readlines()
    ...
    for it in range(0,len(infile),numn):
        ...
        if ident == False :
            ...
            for aa in aas:
                peri += getpercentidentity(arr[0].split('_')[0],aa)
            peri /= len(aas)
            #dis = sum([getdist(aas[q],arr[0].split('_')[0],df) for q in range(k)])/k
            dis = sum([neighbors[q][0] for q in range(k)])/k
            ...
            #add_probs2(probs,it//numn,knns,sstructures,dirnum)
            add_probs2(probs,it//numn,[knns[0]],[sstructures[0]],dirnum)
            #add_probs2(probs,it//numn,[knns[1]],[sstructures[1]],dirnum)
            peri = getpercentidentity(arr[0].split('_')[0],aas[0])
            #dis = getdist(arr[0].split('_')[0],aas[0],df)
            dis = neighbors[0][0] 
